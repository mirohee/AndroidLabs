        ...
        bluetoothGatt = device.connectGatt(this, false, object : BluetoothGattCallback() {
            ...
            override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
                ...
                if (characteristic.uuid == HEART_RATE_MEASUREMENT_CHAR_UUID) {
                    ...
                    heartRateViewModel.addHeartRateMeasurement(heartRateValue)
                }
            }
        })
    }
...
fun MainScreen(
    ...
    var selectedDevice by remember { mutableStateOf<BluetoothDeviceData?>(null) }
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Row {
            ...
            Button(onClick = { showGraph = !showGraph }) {
                Text(if (showGraph) "Hide Graph" else "View Graph")
            }
        }

        selectedDevice?.let { device ->
            Spacer(modifier = Modifier.height(16.dp))
            Text(text = "Selected Device:", color = Color.Black, fontWeight = FontWeight.Bold)
            Text(text = "Name: ${device.name}", color = Color.Black)
            Text(text = "MAC Address: ${device.macAddress}", color = Color.Black)
            Text(text = "RSSI: ${device.rssi}", color = Color.Black)
        }
        ...
        if (showGraph) {
            HeartRateGraph(heartRateViewModel = heartRateViewModel)
        }
...
class HeartRateViewModel : ViewModel() {
    private val _heartRateMeasurements = MutableLiveData<List<Int>>(emptyList())
    val heartRateMeasurements: LiveData<List<Int>> get() = _heartRateMeasurements

    fun addHeartRateMeasurement(value: Int) {
        val updatedList = _heartRateMeasurements.value.orEmpty() + value
        _heartRateMeasurements.postValue(updatedList)
    }
}


@Composable
fun HeartRateChart(values: List<Entry>) {
    AndroidView(
        modifier = Modifier.fillMaxSize(),
        factory = { context ->
            val view = LineChart(context)
            view.setTouchEnabled(true)  // Enable zoom and drag
            view.setPinchZoom(true)     // Enable pinch-to-zoom
        ...
        update = { view ->
            val dataSet = LineDataSet(values, "BPM")
...
@Composable
fun HeartRateGraph(heartRateViewModel: HeartRateViewModel) {
    val heartRateData = heartRateViewModel.heartRateMeasurements.observeAsState(emptyList()).value

    val entries = heartRateData.mapIndexed { index, value ->
        Entry(index.toFloat(), value.toFloat())
    }

    HeartRateChart(values = entries)
}
